...
StatsTransformer.simplify = function (stat) {
  return (0, _find3.default)((0, _keys3.default)(_constants.DEFAULT_TRANSFORM_REGEX), function (key) {
    // no other parameters necessary besides id
    var value = _constants.DEFAULT_TRANSFORM_REGEX[key].regex.test(stat.id) ? key : false;
    if (value) {
      // some types need to check against a type field as well
      if (_constants.DEFAULT_TRANSFORM_REGEX[value].profiler) {
        if (!(stat[_constants.DEFAULT_TRANSFORM_REGEX[value].profiler.type].includes(_constants.DEFAULT_TRANSFORM_REGEX[value].profiler.value) && value.toLowerCase().includes(_constants.DEFAULT_TRANSFORM_REGEX[value].profiler.value))) {
          value = false;
          return value;
        }
      }
